...
async function routes(fastify, options) {
    ...
    fastify.get("/reclamacao/:id", { onRequest: [fastify.authenticatePass] }, async (req, res) => {
      try{
        const reclamacao = await Reclamacao.findByPk(req.params.id, {include: [{model: Categorias}, {model: Departamento, include: [{model: Anexos}]}, { model: Endereco, include: [{model: Cidade}]  }, {model: Likes}, {model: ReclamacaoAnexos, include: [{model: Anexos}]} ]})

        const idUsuario = req.user ? req.user.sub : null
        
        const reclamacoesResponse = {
            ...
            anexos : reclamacao.reclamacao_has_anexos.length > 0 ? reclamacao.reclamacao_has_anexos.map(item => item.anexo.url) : null,
